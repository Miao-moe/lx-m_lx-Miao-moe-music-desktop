...
test('GitHub plugins load independently, release audio resources, and survive offline restart', { timeout: 120000 }, async() => {
  ...
  try {
    ...
    const catalogError = page.getByRole('status').filter({ hasText: await label(page, 'setting__plugins_catalog_error') })
    await catalogError.waitFor()
    assert.match(await catalogError.textContent(), /错误代码：\S+/)
    assert.match(await catalogError.textContent(), /原因：\S+/)
