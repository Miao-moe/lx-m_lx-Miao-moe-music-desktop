...
test('D01-D08: manual backup preview, selective restore, errors and persistence in the production UI', { timeout: 120000 }, async t => {
  ...
  try {
    ...
    await t.test('preview exposes contents and restores only checked playlists', async() => {
      ...
      for (const section of ['settings', 'downloads', 'lyrics', 'plugins', 'library']) await f.page.locator(`[data-backup-section="${section}"]`).uncheck()
      const form = f.page.locator('[data-backup-preview]')
      await form.locator('input[value="default"]').uncheck()
      await form.locator('input[value="love"]').uncheck()
      await form.locator('input[value="temp"]').uncheck()
