...
test('the store imports and exports local packages offline through the real UI and IPC', { timeout: 120000 }, async t => {
  ...
  try {
    ...
    await t.test('bad packages and future APIs report errors and leave the running plugin intact', async() => {
      ...
      await importChosen()
      assert.equal(await status(page).innerText(), await label(page, 'setting__plugins_transfer_incompatible'))
      await choose(app, { incoming: path.join(profilePath, 'missing.zip') })
      await importChosen()
      assert.equal(await status(page).innerText(), await label(page, 'setting__plugins_transfer_read_failed'))
    ...
    await t.test('export refuses the old extension without creating a backup', async() => {
      ...
      await click(page, 'setting__plugins_export', card())
      await idle(page)
      assert.equal(await status(page).innerText(), await label(page, 'setting__plugins_transfer_invalid_destination'))
    ...
    await t.test('a concurrent uninstall invalidates the outstanding confirmation', async() => {
      ...
      await idle(page)
      assert.equal(await status(page).innerText(), await label(page, 'setting__plugins_transfer_changed'))
      assert.equal(await card().count(), 0)
      await importChosen()
      await version('1.1.0')
    })
