...
test('audio tag editor installs in the existing host and edits downloaded and selected files', { timeout: 120000 }, async t => {
  ...
  const task = (id, filePath, complete = true) => ({
    ...
    metadata: { filePath, fileName: path.basename(filePath), musicInfo: { name: id, singer: '鎖那', meta: {} } },
  })
  ...
  try {
    ...
    await setupIpc()
    await page.evaluate(directory => { window.lxData.appSetting['download.savePath'] = directory }, profilePath)
    await openStore(page)
    await install(page, 'audio-tag-editor')
    await page.getByRole('tab', { name: '音频标签编辑', exact: true }).click()
    await settled(page)
    ...
    await t.test('completed supported downloads can be searched and selected', async() => {
      ...
      await page.waitForFunction(() => document.querySelector('[data-plugin-settings="audio-tag-editor"] form input')?.value === 'コーヒーカップ')
    ...
    await t.test('drafts survive settings navigation; reset and canceled selection preserve the current file', async() => {
    ...
    await t.test('the current download directory is used when an old file path no longer exists', async() => {
      await editor.locator('aside button').filter({ hasText: '晚风.flac' }).click()
      await page.waitForFunction(() => document.querySelector('[data-plugin-settings="audio-tag-editor"] form input')?.value === '晚风')
    ...
    await t.test('a local file can be selected and the form fits small and large windows', async() => {
      ...
      await editor.getByRole('button', { name: '选择音频文件', exact: true }).click()
      await page.waitForFunction(() => document.querySelector('[data-plugin-settings="audio-tag-editor"] form input')?.value === '咖啡与晚风').catch(async error => {
      ...
      for (const width of [828, 1280]) {
        ...
        assert.equal(await editor.evaluate(element => element.scrollWidth <= element.clientWidth + 1), true)
    ...
    await setupIpc()
    await openStore(page)
    await t.test('offline restart loads the installed plugin and uninstall removes its interface', async() => {
      await page.locator('[data-plugin-id="audio-tag-editor"]').getByRole('button', { name: await label(page, 'setting__plugins_settings'), exact: true }).click()
      await page.locator('[data-plugin-settings="audio-tag-editor"]').waitFor()
      await uninstall(page, 'audio-tag-editor')
      assert.equal(await page.getByRole('tab', { name: '音频标签编辑', exact: true }).count(), 0)
      assert.equal(await page.locator('[data-plugin-settings="audio-tag-editor"]').count(), 0)
    })
    assert.deepEqual(fixture.errors, [])
