...
test('Downloads opens the tag editor for the clicked file and handles stale files and task state', { timeout: 120000 }, async t => {
  ...
  const { app, page, errors, output } = await launch({ rendererPath })
  ...
  try {
    ...
    await t.test('the menu is available on a fresh installation without visiting the store', async() => {
      ...
      assert.match(await editor.locator('form h4').first().innerText(), /alpha-song\.mp3/)
    ...
    await t.test('the modal close X accepts a real mouse click above the window toolbar', async() => {
      const closeButton = editor.locator('xpath=../..').locator('header button')
      const bounds = await closeButton.boundingBox()
      assert(bounds)
      assert(bounds.width >= 28 && bounds.height >= 28, 'the visible X needs a usable hit target')
      assert.equal(await closeButton.evaluate(button => getComputedStyle(button).getPropertyValue('-webkit-app-region')), 'no-drag')
