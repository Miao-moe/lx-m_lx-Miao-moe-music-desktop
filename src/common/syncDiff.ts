export interface SyncDifference { kind: 'added' | 'removed' | 'renamed' | 'changed', scope: string, before: string, after: string }
...
export const playlistDiff = (before: LX.Sync.List.ListData, after: LX.Sync.List.ListData): SyncDiff => {
  ...
  for (const [id, list] of left) {
    ...
    if (!next) { add({ kind: 'removed', scope: '歌单', before: list.name, after: '' }); continue }
    if (list.name !== next.name) add({ kind: 'renamed', scope: '歌单', before: list.name, after: next.name })
  ...
  for (const [id, list] of right) if (!left.has(id)) add({ kind: 'added', scope: '歌单', before: '', after: list.name })
  return result
}
