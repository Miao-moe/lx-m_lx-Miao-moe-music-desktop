export const downloadFailureKinds = ['network', 'timeout', 'permission', 'disk', 'limit', 'http', 'url', 'conflict', 'postprocess', 'unknown'] as const
...
export const classifyDownloadError = (error: { code?: string, statusCode?: number, message?: string } = {}): DownloadFailureKind => {
  if (['DOWNLOAD_TASK_SIZE_LIMIT', 'DOWNLOAD_BATCH_SIZE_LIMIT'].includes(error.code ?? '')) return 'limit'
