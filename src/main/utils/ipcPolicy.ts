...
checks.set('winMain_webdav_browse', str)
checks.set('winMain_webdav_audio_url', shape({ path: str, identity: str }))
...
checks.set('winMain_open_api_action', v => obj(v) && (v.action === 'status' || (v.action === 'enable' && shape({ enable: bool, bindLan: bool, port: v => typeof v === 'string' && /^\d{1,5}$/.test(v) && +v > 0 && +v <= 65535 })(v.data))))
...
add('player_', 'list_music_get list_music_clear list_music_get_list_ids', str)
add('player_', 'list_remove list_trash_restore list_trash_delete', strings)
