...
const resolveEntity = async(type: EntityType, source: LX.OnlineSource, summary: EntitySummary) => {
  ...
  const resolvedEntity = {
    ...
    summary: {
      ...
      desc: info.desc ? info.desc : summary.desc,
...
const normalizeResult = (result: RawDetailResult, type: EntityType, id: string, page: number, summary: EntitySummary, isFallback: boolean): EntityDetailInfo => {
  ...
  return {
    ...
    isFallback,
    info: {
      name: result.info?.name ? result.info.name : summary.name,
      author: result.info?.author ? result.info.author : summary.author,
      img: result.info?.img ? result.info.img : summary.img,
      desc: result.info?.desc ? result.info.desc : summary.desc,
